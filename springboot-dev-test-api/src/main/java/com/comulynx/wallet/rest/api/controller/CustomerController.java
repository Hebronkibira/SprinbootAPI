...
public class CustomerController {
    ...
    public ResponseEntity<?> createCustomer(@RequestBody Customer customer) {
        try {
            Optional<Customer> exist = customerRepository.findByCustomerId(customer.getCustomerId());
            if (exist.isPresent()) {
                return new ResponseEntity<>("A Customer Exists with id ::" + customer.getCustomerId(), HttpStatus.OK);
            } else {
                // Exception.
                int strength = 10;
                BCryptPasswordEncoder bCryptPasswordEncoder
                        = new BCryptPasswordEncoder(strength, new SecureRandom());
                String encodedPassword = bCryptPasswordEncoder.encode(customer.getPin());
                customer.setPin(encodedPassword);
